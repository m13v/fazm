...
open class Highlightr
{
    ...
    public init?(highlightPath: String? = nil)
    {
        ...
        // Look for resource bundle in Bundle.main.resourceURL first (correct for macOS .app bundles),
        // then fall back to Bundle.module (works for flat SPM builds and command-line tools).
        let bundle: Bundle
        if let resourceURL = Bundle.main.resourceURL,
           let b = Bundle(path: resourceURL.appendingPathComponent("Highlightr_Highlightr.bundle").path) {
            bundle = b
        } else {
            #if SWIFT_PACKAGE
